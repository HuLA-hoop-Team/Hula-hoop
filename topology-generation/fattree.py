...
def mk_topo(pods, bw='1Gbps'):
    ...
    a.add_subgraph(map(lambda x: x[0], hosts), rank='same')
    a.add_subgraph(map(lambda x: x[0], core_switches), rank='same')
    a.add_subgraph(map(lambda x: x[0], agg_only_switches), rank='same')
    a.add_subgraph(map(lambda x: x[0], edge_switches), rank='same')
...
if __name__ == '__main__':
    ...
    if args.output:
        ...
        links = topo.edges()
        hosts = list(filter(lambda n: n.startswith('h'), topo.nodes()))
        sws = list(filter(lambda n: n.startswith('s'), topo.nodes()))
        switches = {}
        for switch in sws:
            switches[switch] = {}
